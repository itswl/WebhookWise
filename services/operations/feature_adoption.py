...
_ADOPTION_ERRORS = (RedisError, RuntimeError, OSError, TimeoutError, TypeError, ValueError)

# Hard budget: adoption accounting is awaited inline on request paths, so a
# sick Redis must cost a request at most this much, not its socket timeout.
_RECORD_BUDGET_SECONDS = 0.25
...
async def _record(feature: str, now: datetime | None) -> None:
...
async def record_feature_use(feature: str, *, now: datetime | None = None) -> None:
    """Increment this month's counter for `feature`; never raises, hard-budgeted."""
    try:
        await asyncio.wait_for(_record(feature, now), timeout=_RECORD_BUDGET_SECONDS)
...
async def get_feature_adoption(*, now: datetime | None = None) -> dict[str, Any]:
    ...
    for month_stamp in (stamp, prev_month_stamp):
        ...
        try:
            raw = await redis_client.get_redis().hgetall(_month_key(month_stamp))
        except _ADOPTION_ERRORS as e:
