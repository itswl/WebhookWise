...
**方式 1：通过 API**
```bash
curl -X POST https://webhookwise.internal.example/api/migrations/add_unique_constraint
```
...
```bash
# 1. 检查告警状态
curl -s "https://webhookwise.internal.example/api/webhooks/8928" | jq '.data | {id, is_duplicate, duplicate_count}'
# 输出: {"id":8928, "is_duplicate":0, "duplicate_count":2}

curl -s "https://webhookwise.internal.example/api/webhooks/8930" | jq '.data | {id, is_duplicate, duplicate_of}'
