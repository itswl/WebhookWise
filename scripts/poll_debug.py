...
gateway_url = (
    os.getenv("OPENCLAW_GATEWAY_URL", "http://127.0.0.1:18900")
    .replace("http://", "ws://")
    .replace("https://", "wss://")
    + "/ws"
)
    ...
    if frame.get("type") == "event" and frame.get("event") == "connect.challenge":
        ...
        if device_id and private_key_b64 and nonce:
            try:
                ...
                payload_str = (
                    f"v2|{device_id}|gateway-client|cli|operator|{scopes_str}|{signed_at}|{gateway_token}|{nonce}"
                )
        ...
        if messages:
            ...
            for i, m in enumerate(messages):
                ...
                if isinstance(content, list):
                    ...
                    tlen = sum(
                        len(c.get("text", "")) for c in content if isinstance(c, dict) and c.get("type") == "text"
                    )
                else:
                    types = ["str"]
            ...
            if "payload" in data:
                print(
                    "Payload keys:",
                    list(data["payload"].keys()) if isinstance(data["payload"], dict) else type(data["payload"]),
                )
