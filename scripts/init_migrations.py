#!/usr/bin/env python3
"""
启动时自动执行的数据修复检查（仅保留数据修复函数）。

DDL 变更已统一收敛到 Alembic 迁移，此模块仅负责：
1. check_and_add_unique_constraint — 修复重复原始告警并创建唯一索引
2. fix_duplicate_count — 同步重复告警的 duplicate_count 字段
"""
