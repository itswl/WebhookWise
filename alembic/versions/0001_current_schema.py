...
def upgrade() -> None:
    ...
    op.create_index("ix_webhook_events_dedup_key", "webhook_events", ["dedup_key"])
    ...
    op.create_index("ix_ai_usage_logs_alert_hash", "ai_usage_logs", ["alert_hash"])
    op.create_index("idx_ai_usage_logs_timestamp_route", "ai_usage_logs", ["timestamp", "route_type"])
    ...
    op.create_table(
        ...
        sa.Column("match_project", sa.String(length=200), nullable=False),
        sa.Column("match_region", sa.String(length=200), nullable=False),
        sa.Column("match_environment", sa.String(length=200), nullable=False),
    ...
    op.create_table(
        ...
        sa.Column("forward_data", jsonb, nullable=True),
        sa.Column("analysis_result", jsonb, nullable=True),
        sa.Column("formatted_payload", jsonb, nullable=True),
    ...
    op.create_index("ix_forward_outboxes_webhook_event_id", "forward_outboxes", ["webhook_event_id"])
    op.create_index("ix_forward_outboxes_status", "forward_outboxes", ["status"])
    op.create_index("idx_forward_outboxes_event", "forward_outboxes", ["webhook_event_id"])
    op.create_index(
    ...
    op.create_table(
        ...
        sa.Column("related_alert_ids", jsonb, nullable=False),
        sa.Column("confidence", sa.Float(), nullable=False),
        sa.Column("created_at", sa.DateTime(), nullable=True),
        sa.PrimaryKeyConstraint("id"),
    )
    op.create_index("idx_suppressed_records_created_at", "suppressed_records", ["created_at"])
    op.create_index("idx_suppressed_records_hash_created", "suppressed_records", ["alert_hash", "created_at"])
