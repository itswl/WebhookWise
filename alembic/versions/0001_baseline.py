...
def upgrade() -> None:
    ...
    op.create_index(
        op.f("ix_archived_webhook_events_alert_hash"), "archived_webhook_events", ["alert_hash"], unique=False
    )
    op.create_index(
        op.f("ix_archived_webhook_events_archived_at"), "archived_webhook_events", ["archived_at"], unique=False
    )
    op.create_index(
        op.f("ix_archived_webhook_events_request_id"), "archived_webhook_events", ["request_id"], unique=False
    )
    op.create_index(
        op.f("ix_archived_webhook_events_timestamp"), "archived_webhook_events", ["timestamp"], unique=False
    )
    op.create_table(
        "forward_rules",
    ...
    op.create_index(
        "idx_suppressed_records_hash_created", "suppressed_records", ["alert_hash", "created_at"], unique=False
    )
    op.create_table(
        "webhook_events",
        sa.Column("id", sa.Integer(), autoincrement=True, nullable=False),
    ...
    op.create_index(
        op.f("ix_forward_outboxes_webhook_event_id"), "forward_outboxes", ["webhook_event_id"], unique=False
    )
...
def downgrade() -> None:
    ...
    op.drop_index(
        "idx_deep_analyses_pending_next_poll",
        table_name="deep_analyses",
        postgresql_where=sa.text("status = 'pending'"),
    )
    op.drop_index(
        "idx_deep_analyses_pending", table_name="deep_analyses", postgresql_where=sa.text("status = 'pending'")
    )
