...
```bash
docker compose -p webhookwise-observability --env-file .env -f deploy/compose/docker-compose.observability.yml restart grafana
```
...
| Faro (dormant — frontend SDK removed in 3.7.x) | `faro_receiver_events_total`, `faro_receiver_measurements_total`, `faro_receiver_exceptions_total`, `faro_receiver_logs_total` | Alloy component labels |
