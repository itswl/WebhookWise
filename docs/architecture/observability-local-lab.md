...
代码里使用 OpenTelemetry 点号命名，例如 `http.server.requests`。进 Prometheus 后通常会变成下划线命名，例如 `http_server_requests_total`。本地 Prometheus 里有些应用指标也会出现 `webhookwise_` 前缀版本；日常排查优先用无前缀业务名，查不到时再试前缀版本。

### HTTP / API 指标

| 指标 | 类型 | 关键标签 | 含义 | 异常解读 |
| --- | --- | --- | --- | --- |
| `http_server_requests_total` | Counter | `http_method`、`http_route`、`http_status_code`、`service_name` | API 请求总数 | 5xx 升高说明服务端错误；4xx 升高多半是参数、认证、路由或调用方问题 |
| `http_server_request_duration_seconds_bucket` | Histogram | `http_method`、`http_route`、`http_status_code` | API 请求耗时分布 | p95/p99 升高时，继续看 Trace、DB、Redis、AI 和 Forwarding 指标 |
| `http_server_request_body_size_bytes_bucket` | Histogram | `http_method`、`http_route` | 请求体大小 | Webhook payload 突然变大时，可能导致解析、入库、AI 分析变慢 |
...
`webhook_received_total` 是入口吞吐，`http_server_requests_total` 是 HTTP 层吞吐。两者不一定完全相等，因为 HTTP 层还包含 ready、dashboard、静态资源或其他接口。
...
| `webhook_running_tasks_ratio` | Gauge | 通常无业务标签 | 当前运行中的 webhook 任务数 | 持续高位说明 worker 忙；高位叠加 queue lag 是处理能力不足信号 |

注意：`webhook_running_tasks_ratio` 后缀叫 `ratio`，但本地导出里代表当前运行任务数，不是百分比。
...
| `db_pool_connections_checked_out_ratio` | Gauge | 通常无业务标签 | 当前借出的 DB 连接数 | 长期接近连接池上限，说明 DB pool 压力大 |
| `db_pool_connections_max_ratio` | Gauge | 通常无业务标签 | DB 连接池容量 | 和 checked_out 一起看连接池是否打满 |
