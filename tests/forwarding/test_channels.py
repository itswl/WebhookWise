...
def test_deep_analysis_record_resolves_to_openclaw_channel() -> None:
    ch = resolve_channel(_record(target_type="deep_analysis"))
    assert ch.name == "deep_analysis"
...
def test_gateway_takes_precedence_over_feishu_url() -> None:
    # An openclaw record should never be treated as feishu even if its URL looked feishu-ish.
    ch = resolve_channel(_record(target_type="deep_analysis", target_url="https://open.feishu.cn/x"))
    assert ch.name == "deep_analysis"


def test_gateway_followup_only_on_pending_result() -> None:
    ch = resolve_channel(_record(target_type="deep_analysis"))
    assert (
        ch.needs_followup_on_success(_record(target_type="deep_analysis"), {"status": "pending", "_pending": True})
        is True
    )
    assert ch.needs_followup_on_success(_record(target_type="deep_analysis"), {"status": "success"}) is False
