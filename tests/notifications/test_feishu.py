...
def test_build_feishu_card_formats_identity_and_periodic_reminder() -> None:
    ...
    assert "🏷️ demo-cn/cn-shanghai · webhook-api · api-0 i-001 · HighCPU · cpu_usage · critical" in rendered
...
def test_build_deep_analysis_card_formats_root_cause_json_report() -> None:
    ...
    report = {
        ...
        "summary": "ES集群 demo-cn-prod-es 自 2026-06-03 23:38 CST 起处于 yellow 状态。",
        "root_cause": {
            "status": "confirmed",
    ...
    assert "ES集群 demo-cn-prod-es" in rendered
    assert "数据节点约 12.3 小时前发生重启" in rendered
    assert "副本分片未完全恢复" in rendered
    assert "检查重启节点 JVM/磁盘水位" in rendered
    assert "项目: demo-cn" in rendered
    assert "资源: demo-cn-prod-es" in rendered
...
def test_card_strips_redundant_impact_and_summary_prefix() -> None:
    ...
    card = build_feishu_card(
        {"source": "volcengine", "timestamp": "2026-06-18T01:00:00Z", "parsed_data": {}},
        {
            "importance": "high",
            "summary": "摘要：桶错误率 36%",
            "impact_scope": "影响范围：存储桶 flink-meta，可能影响 checkpoint 保存。",
        },
    )
    rendered = str(card)
    ...
    assert "存储桶 flink-meta，可能影响 checkpoint 保存。" in rendered
