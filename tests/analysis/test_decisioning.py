...
class TestRuleMatches:
    ...
    def test_project_region_environment_filters(self) -> None:
        rule = _make_rule(match_project="demo-cn", match_region="cn-shanghai", match_environment="prod,production")
        payload = {
            "Resources": [
                {
                    "ProjectName": "demo-cn",
                    "Region": "cn-shanghai",
                    "Name": "demo-cn-prod-mongo",
                }
            ]
        }
    ...
    def test_project_environment_exclusion_filters(self) -> None:
        prod_payload = {"Resources": [{"ProjectName": "demo-cn", "Name": "demo-cn-prod-api"}]}
        dev_payload = {"Resources": [{"ProjectName": "demo-web-cn", "Name": "demo-web-cn-dev-api"}]}
        ...
        assert not _rule_matches(_make_rule(match_project="!demo-cn,!sample-cn"), parsed_data=prod_payload)
        assert _rule_matches(_make_rule(match_project="!demo-cn,!sample-cn"), parsed_data=dev_payload)

    def test_extract_forward_match_fields_prefers_explicit_environment(self) -> None:
        fields = extract_forward_match_fields(
            {
                "Resources": [{"ProjectName": "sample-cn", "Region": "cn-shanghai", "Name": "resource-dev"}],
