...
- **运行时策略中心化**: 动态策略从数据库 `system_configs` 热更新，并通过 Redis Pub/Sub 实时同步到各进程内存（统一通过 `Config.*` 读取），消除“到底来自 .env 还是 DB”的歧义。
- **多分析引擎适配**: 完美兼容 **Hermes (HMAC 签名)** 与 **OpenClaw (Bearer Token)** 双套深度分析协议。

## ✨ 功能特性

### 核心能力
- ✅ **异步 Webhook 接收**: 采用 **TaskIQ** 异步任务队列，立即返回 202 Accepted，后台处理分析逻辑。
